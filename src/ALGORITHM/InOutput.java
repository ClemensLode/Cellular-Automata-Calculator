...
public class InOutput {
     ...
     * @throws Exception if the significant rule number could not be created
     */
    public static String getImageFileName(boolean injective, boolean surjective) throws Exception {
        return "graphs/" + Function.getSignificantWolframRuleNumber() + "_" + Neighborhood.getShortSignificantNeighborhoodString() + "_" + CellStates.getNumberOfCellStates() + (injective ? "_i" : "") + (surjective ? "_s" : "");
    }

    /**
     * Generate a file name for the graph output on basis of the comitted parameter list
    ...
    public static String getImageFileName(Object[] parameter) {
        return "graphs/" + (BigInteger) parameter[0] + "_" + ((String) parameter[3]).replace(" ", "").replace(",", "-") + "_" + (Integer) parameter[6] + (((Boolean) parameter[7]) ? "_i" : "") + (((Boolean) parameter[8]) ? "_s" : "");
    }

    /**
     * Generate a file name for the simulation output on basis of the comitted parameter list
     * @param parameter Object list in the usual order (BigInteger, String, Integer, Integer, Integer, Boolean, Boolean, Boolean)
     * @return the file name
     */
    public static String getSimulationFileName(Object[] parameter) {
        return "simulations/" + (BigInteger) parameter[0] + "_" + ((String) parameter[3]).replace(" ", "").replace(",", "-") + "_" + (Integer) parameter[6] + (((Boolean) parameter[7]) ? "_i" : "") + (((Boolean) parameter[8]) ? "_s" : "") + ".sim.png";
    }
    ...
    public static Object[] parseParametersAsString(String parameters) throws NumberFormatException {
        Object[] l = new Object[GUI.ResultsTable.COLUMN_COUNT];
    ...
    /**
     * Generates an Object array out of the rule numbers, neighborhood string and sizes and cell states
     * @param generate_graph if the entry has an corresponding graph file
     * @return Object array
     */
     ...
     * @param time time we want to display
    ...
    /**
     * Translates the value 'needed_memory' to a string by converting it in the standard B/KB/MB/GB format.
     * @param needed_memory needed memory in Bytes
