 ...
 *
 * The difference to the class FastBruijnProductGraph is that the calculation
 * will not be canceled early when the result is determined. This is done in
 * order to gather all information for a complete picture of the Bruijn product
 * graph.
 */
public class PrintBruijnProductGraph extends BruijnProductGraph {
	 ...
	 *            product graph. (y coordinate)
	 * @param my_ax_x
	 *            Second of two values that encode the position in the Bruijn
	 *            product graph. (x coordinate)
	 */
	public PrintBruijnProductGraph(final int my_ax_y, final int my_ax_x) {
	 ...
	 */
	@Override
	protected PrintBruijnProductGraph createNewNode(final int my_ax_y,
			final int my_ax_x) {
	 ...
	 * 
	 * @return always true, we can't cancel early because for the graph we want
	 *         to visit all nodes
	 */
	@Override
	protected boolean testNodeStronglyConnected() {
		if (low_id == id) {
			...
			final List<BruijnProductGraph> component = new java.util.LinkedList<BruijnProductGraph>();
			if (this == stack.peek()) {
				stack.pop();
				inStack = false;
				if (ax_x == ax_y) {
				...
				if (isDiagonalElement) {
					...
					/*
					 * the strongly connected component contains diagonal
					 * elements as well as non-diagonal elements Then the whole
					 * diagonal is not a single strongly connected component and
					 * we can cancel early
					 */
	...
	/**
	 * Checks if corresponding graph entry is null and creates a new node if
	 * that's the case
	 * 
	 * @param index
	 *            index to test
	 ...
	 *            y coordinate
	 */
	protected void check_for_null(final int index, final int x, final int y) {
		if (graph[index] == null) {
	...
	/**
	 * Traverses through the whole Bruijn product graph to test if it's
	 * surjective and/or injective
	 * 
	 * @throws Exception
	...
	public static void runConnectionTest() throws Exception {
		...
		for (int i = 0; i < last_element_index; i++) {
			final int index = i * last_element_index + i;
			if (graph[index] == null) {
	...
	/**
	 * Writes the current Bruijn product graph into a file using the viz file
	 * format
	 * 
	 * @param file_name
	 *            Base file name of the .viz file
	 * @throws Exception
	 *             I/O Error writing file
	 */
	public static void printToFile(final String file_name) throws Exception {
		FileOutputStream f;
		PrintStream p;
		try {
		...
		try {
			...
			p.println("size=\"" + 4 * Function.getMaxArraySize() + "," + 4
					* Function.getMaxArraySize() + "\"");
			final String color_list[] = { "black", "blue", "brown", "green",
					"darkgreen", "darkorange", "gold", "hotpink", "purple",
					"turquoise", "magenta" };
			...
			final Iterator it = stronglyConnectedComponentsList.iterator();
			while (it.hasNext()) {
				final List component = (List) it.next();
				if (component.size() == 1) {
					p.print("node [shape = circle, color = "
							+ color_list[color % 11] + "]; ");
				} else {
					p.print("node [shape = doublecircle, color = "
							+ color_list[color % 11] + "]; ");
				}
				color++;
				final Iterator j = component.iterator();
				while (j.hasNext()) {
					final PrintBruijnProductGraph k = (PrintBruijnProductGraph) j
							.next();
			...
			for (int i = 0; i < last_element_index; i++) {
				for (int j = 0; j < last_element_index; j++) {
					...
					for (int b = 0; b < cell_states; b++) {
						axb_i[b] = graph[i * last_element_index + j].ax_y
								* cell_states + b;
						axb_j[b] = graph[i * last_element_index + j].ax_x
								* cell_states + b;
						xb_i[b] = axb_i[b] % last_element_index;
						xb_j[b] = axb_j[b] % last_element_index;
					}
					// TODO possible here much of the stack [local variables]
					// can be saved.
					for (int bi = 0; bi < CellStates.getNumberOfCellStates(); bi++) {
						for (int bj = 0; bj < CellStates
								.getNumberOfCellStates(); bj++) {
							if (Function.getFunction(axb_i[bi]) == Function
									.getFunction(axb_j[bj])) {
								final int index = xb_i[bi] * last_element_index
										+ xb_j[bj];
								if (last_element_index < 26) {
									p.print("" + (char) ('A' + i));
								} else {
								...
								if (last_element_index < 26) {
									p.print(""
											+ (char) ('A' + graph[index].ax_x));
								} else {
									p.print("" + graph[index].ax_x);
								}

								p.println(" [ label = \""
										+ Function.getFunction(axb_i[bi])
										+ "\" ];");
							}
						}
					}
			...
			throw new Exception("printToFile(): Error writing to file "
					+ file_name + " [" + e + "]");
		}
	}
}
