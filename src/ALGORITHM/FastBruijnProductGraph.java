 ...
 *
 * The difference to the class PrintBruijnProductGraph is that the calculation
 * will be canceled early when the result is determined.
 */
public class FastBruijnProductGraph extends BruijnProductGraph {
	 ...
	 */
	public FastBruijnProductGraph(final int my_ax_y, final int my_ax_x) {
	 ...
	 */
	@Override
	protected FastBruijnProductGraph createNewNode(final int my_ax_y,
			final int my_ax_x) {
	 ...
	 * 
	 * @return false if we determined that we can cancel early, true otherwise
	 */
	@Override
	protected boolean testNodeStronglyConnected() {
		...
		if (low_id == id) {
			/*
			 * Ignore the trivial case (a node that is not connected to any
			 * other node). If this is a diagonal element we can cancel early
			 */
			if (this == stack.peek()) {
				stack.pop();
				inStack = false;
				...
				// reset max_stack counter, only important for communication
				// with the GUI
				max_stack = 128;

				/*
				 * diagonalElements represent the number of elements of this
				 * strongly connected componentthat are not diagonal elements
				 */
				...
				if (isDiagonalElement) {
					// were all elements of the diagonal part of the strongly
					// connected component?
					if (nonDiagonalElements == 0) {
						is_surjective = 1;
					} /*
					 * Only a part of the diagonal is a part of a strongly
					 * connected component? Then the whole diagonal is not a
					 * single strongly connected component and we can cancel
					 * early
					 */else {
						is_surjective = 0;
						is_injective = 0;
				...
				/*
				 * we can cancel the calculation early if we know that the
				 * diagonal is a strongly connected component and that there is
				 * at least another strongly connected component
				 */
	...
	public static void runConnectionTest() throws Exception {
		...
		// first check diagonal, if the diagonal isn't a strongly connected
		// component we can cancel the calculation early
		for (int i = 0; i < Function.getLastElementIndex(); i++) {
			final int index = i * Function.getLastElementIndex() + i;
			if (graph[index] == null) {
				graph[index] = new FastBruijnProductGraph(i, i);
			}

			if (graph[index].id == -1 && !graph[index].visit()) {
				break;
			}
		}

		for (int i = 1; i < Function.getLastElementIndex()
				* Function.getLastElementIndex(); i++) {
			if (graph[i] == null) {
				graph[i] = new FastBruijnProductGraph(k, l);
			}
			if (graph[i].id == -1 && !graph[i].visit()) {
				break;
			}
			l++;
