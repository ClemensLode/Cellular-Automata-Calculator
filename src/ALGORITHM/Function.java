...
public class Function {
    ...
    /**
     * the number of neighborhood configurations of a cell of the significant function
     */
    private static int significantMaxArraySize = 0;
    /**
     * the function that maps each neighborhood configuration of the significant neighborhood to a value
     */
    ...
    /**
     * updates the last element index and the max Array size, reinitialized the function
     */
    private static void updateMaxArraySize() {
        lastElementIndex = Misc.pow(CellStates.getNumberOfCellStates(), Neighborhood.getNeighborhoodSize() - 1);
        maxArraySize = lastElementIndex * CellStates.getNumberOfCellStates();
        function = new int[getMaxArraySize()];
    }
    
    /**
     * Calculates the number of possible neighborhood configurations a cell can have
     ...
     */
    public static int calculateMaxArraySize(int cell_states, int neighborhood_size) {
        return Misc.pow(cell_states, neighborhood_size);
    }

    /**
     * updates the significant max array size, i.e. the number of possible neighborhood configurations of the significant neighborhood
     */
    private static void updateSignificantMaxArraySize() {
        significantMaxArraySize = calculateMaxArraySize(CellStates.getNumberOfCellStates(), Neighborhood.getSignificantNeighborhoodSize());
     ...
     * @throws Exception when the rule number is out of bounds
    ...
    public static void createSignificantFunction(BigInteger rule_nr) throws Exception {
        ...
        significantFunction = calculateFunction(rule_nr, getSignificantMaxArraySize(), getMaxArraySize(), CellStates.getNumberOfCellStates());
        
        Neighborhood.resetSignificantNeighborhoodSize();
    }
    
    /**
     * Determines the function array out of a wolfram rule number and the corresponding parameters
     * @param rule_nr Wolfram rule number
     * @param significant_max_array_size number of possible neighborhood configurations of the significant neighborhood
     * @param max_array_size number of possible neighborhood configurations of the neighborhood
     * @param cell_states number of cell states
     * @return function array 
     * @throws java.lang.Exception if the rule number parameter is out of range
     */
    public static int[] calculateFunction(BigInteger rule_nr, final int significant_max_array_size, final int max_array_size, final int cell_states) throws Exception {
        int[] new_function = new int[significant_max_array_size]; 
        BigInteger max_rule_number = WolframRule.getMaxWolframRuleNumber(max_array_size, cell_states);
        if ((rule_nr.compareTo(BigInteger.ZERO) < 0) || (rule_nr.compareTo(max_rule_number) >= 0)) {
            throw new Exception("createSignificantFunction(): wolfram rule number " + rule_nr + " out of range (not within [0, " + max_rule_number.toString() + "]).");
        }
        for (int i = 0; i < significant_max_array_size; i++) {
            new_function[i] = rule_nr.mod(BigInteger.valueOf(cell_states)).intValue();
     ...
     * @throws Exception when creating / accessing the significant function (TODO)
     */
    public static void createStandardSignificantFunction() throws Exception {
        updateSignificantMaxArraySize();
        for (int j = 0; j < CellStates.getNumberOfCellStates(); j++) {
            for (int i = 0; i < getSignificantMaxArraySize() / CellStates.getNumberOfCellStates(); i++) {
                significantFunction[i + j * getSignificantMaxArraySize() / CellStates.getNumberOfCellStates()] = j;
            }
        }
    }
     ...
     * @throws Exception if an internal error occured
     */
    public static void updateFunction() throws Exception {
        ...
        for (int i = 0; i < getMaxArraySize(); i++) {
            ...
            // transform bit array with getSignificantNeighborhoodPosition to new bit array
            // use new bit array as new z
            
            int z = i;
            int t = 0;
            int[] bit_array = new int[Neighborhood.getNeighborhoodSize()];
            for(int k = 0; k < bit_array.length; k++) {
                bit_array[k] = z % CellStates.getNumberOfCellStates();
                z /= CellStates.getNumberOfCellStates();
            }
            
            int[] transformed_bit_array = new int[Neighborhood.getSignificantNeighborhoodSize()];
            int[] sig_bit_array = Neighborhood.getStandardizedSignificantNeighborhoodCopy();
    ...
    /**
     * @return the Wolfram rule number based on the current significant function and the number of cell states
     */
    public static BigInteger getSignificantWolframRuleNumber() {
        return WolframRule.getWolframRuleNumber(significantFunction, CellStates.getNumberOfCellStates());
    }

    /**
     * @return the polynomial rule based on the current significant function and the number of cell states
     */
    public static String getSignificantPolynomialRule() {
        return PolynomialRule.getPolynomialRule(significantFunction, CellStates.getNumberOfCellStates());
    }
    ...
    public static int getSignificantFunction(int i) throws Exception {
        if ((i < 0) || (i >= significantFunction.length)) {
            throw new Exception("getSignificantFunction(): index " + i + " out of range (" + significantFunction.length + ")");
        }
        return significantFunction[i];
    }

    /**
     * Calculates the result of the function applied on the neighborhood of a cell
     * @param neighbors the neighborhood of the cell
     * @return the corresponding value of the function
     * @throws java.lang.Exception see Misc.getNeighborhoodFunction
     */
    public static int getSignificantFunction(int[] neighbors) throws Exception {
        return Misc.getNeighborhoodFunction(neighbors, significantFunction, CellStates.getNumberOfCellStates());
    }
