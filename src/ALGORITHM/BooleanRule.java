...
public class BooleanRule {
     ...
     * @param function Function that we want to transform into a boolean rule string
        ...
        for (int l = 0; l < neighborhood_size; l++) {
            ...
            for (int i = 0; i < significant_max_array_size; i++) {
                ...
                for (int j = i + 1; j < significant_max_array_size; j++) {
                    ...
                    // shorten the boolean rule by combining and replacing with more general rule parts
...
// now create the actual boolean rule string out of the set_bits array
            ...
            for (int i = 0; i < significant_max_array_size; i++) {
                ...
                if (!empty) {
                    ...
                    for (int k = 0; k < neighborhood_size; k++) {
                        ...
                        boolean_rule = boolean_rule + "x" + (Neighborhood.getNeighborhoodNumber(k) + 1);//(k+1);//Neighborhood.getSignificantNeighborhoodPosition(k);
                        if (set_bits[i][k] == 0) {
     ...
     * @param boolean_rule The boolean rule string to be parsed
     * @param significant_max_array_size Number of neighborhood combinations of the function
     * @return The Wolfram Rule Number
     * @throws java.lang.Exception If there was a parse error, i.e. an invalid boolean rule
     */
    public static BigInteger extractBooleanRuleNumber(String boolean_rule, final int significant_max_array_size) throws Exception {
...
//       check for "false"/"true"
        ...
        if (true_index > 0) {
            ...
            return WolframRule.getMaxWolframRuleNumber(significant_max_array_size, 2).subtract(BigInteger.ONE);
        }
        ...
        ArrayList<ArrayList<Integer>> xe_list = new ArrayList<ArrayList<Integer>>();
        ArrayList<ArrayList<Boolean>> not_list_list = new ArrayList<ArrayList<Boolean>>();
        ...
        for (String s : or_elements) {
            ...
            if (s.compareTo("b1") == 0) {
                // return max wolfram number because an OR element is 1, is always true
                return WolframRule.getMaxWolframRuleNumber(significant_max_array_size, 2).subtract(BigInteger.ONE);
            }
            ...
            ArrayList<Boolean> not_list = new ArrayList<Boolean>();
            ...
            for (String f : not_elements) {
                ...
                for (int x = 0; x < x_elements.length; x++) {
                    ...
                    if (x_value <= 0) {
                        throw new Exception("extractBooleanRuleNumber(): First neighborhood / variable index is 1, not 0");
                    }
        ...
        if (neighborhood_size > Neighborhood.getNeighborhoodSize()) {
            throw new Exception("extractBooleanRuleNumber(): Implied neighborhood size (" + neighborhood_size + ") exceeds given neighborhood size (" + Neighborhood.getNeighborhoodSize() + ")");
        }
        ...
        for (ArrayList<Integer> l : xe_list) {
            ...
            int n = 0;
            // move through all elements of the xe_list and check corresponding elements from the not_list if the entry is negated
            for (Integer i : l) {
